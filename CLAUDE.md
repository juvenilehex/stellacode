...
## 문서 지도 (`docs/`) — 새 문서를 만들기 전에 여기부터 본다

> **2026-08-25 신설** (Park 지시: "된거라던가 너무낡은거라던가 매 실행마다 자꾸 문서를
> 만드니까 이걸 흡수, 통합"). 아래 문서들은 진입점 어디에서도 이름이 안 불려 **색인에서
> 도달할 수 없었다.** 못 찾으면 다음 세션이 같은 주제를 다시 조사하고 문서를 또 만든다.
> 측정: `python PJ00_develop/dev_projects/tools/doc_index_audit.py <이 프로젝트>`
>
> **규약**: 새 조사·설계·앵커를 쓰면 `docs/` 에 만들고 **그 자리에서 이 표에 한 줄 추가한다.**
> 등재 안 된 문서는 없는 문서다. 주제에 이미 소유자가 있으면 새 파일 말고 그 문서를 고친다.
