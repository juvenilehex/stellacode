...
function NodeCluster({ nodes, geometry }: { nodes: GraphNode[]; geometry: 'sphere' | 'octahedron' }) {
  ...
  useFrame(({ clock }) => {
    ...
    // First frame: initialize all instance colors
    if (!initedRef.current) {
      for (let i = 0; i < count; i++) {
        _color.set(getNodeColor(nodes[i].type, nodes[i].language));
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (isFiltered || (lodLevel === 'project' && node.type === 'file'
      ...
      // Dynamic color: dim non-connected when selection active, apply node opacity
      const isDimmed = hasSelection && !isSelected && !isConnected;
      const dimFactor = isDimmed ? 0.2 : isActive ? 1.0 + 0.5 * sig : nodeOpacity;
