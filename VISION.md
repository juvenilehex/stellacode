...
분석 도구가 아니라 코드 아쿠아리움. 에이전트를 다루는 일 자체로 이미 피곤하다. StellaCode는 쉬면서 보는 것이다. 내 코드 우주를 띄워놓고 구경하다가, "어 저 별 왜 빨개?" 하면서 자연스럽게 알게 되는 것.

기능이 아니라 경험이 우선한다. 시각적으로 아름다운 것이 분석적으로 유용한 것보다 먼저 온다.

## Roadmap

### v1.0 -- Complete (Current)

핵심 파이프라인부터 시각 경험까지 전부 구현 완료.

**Foundation**: Scanner, Parser (TS/JS/Python regex), Graph builder, Force layout, R3F 3D, WebSocket, Git intelligence, AI 에이전트 감지 (6종), UI 패널.

**Refinement**: UI overhaul, InstancedMesh (1000+ 노드), 색상 커스터마이징, Legend 필터, 패널 시스템, Bloom 조정.
...
**Agent Trails**: 에이전트별 궤적 트레일, Agent color mode.

**Polish**: 3-point cinematic lighting, 4-layer starfield, Nebula clouds, Shooting stars, Observe mode, Capture (PNG export).

**Self-Evolution**: 자가발전로직 3라운드 실행. 6-lens 평가 체계 (Rendering, Interaction, Visual, Code, Features, Orders). 최종 점수 57/60 (9.5/10).

### Future

핵심 경험은 완성. 이후는 필요에 따라.
...
코드베이스는 우주와 같다. 너무 크고, 끊임없이 변하고, 한 사람이 전부 이해할 수 없다. 하지만 관측할 수는 있다. 그리고 그건 피곤한 일이 아니라 아름다운 일이다.
